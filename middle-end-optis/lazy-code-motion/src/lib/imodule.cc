...
bool is_branch(const Ins &ins) {
  ...
  return opname == "b" || opname == "cbr" || opname == "ret";
}
...
void BasicBlock::check() const {
  for (std::size_t i = 0; i < _ins.size(); ++i) {
    ...
    if (op == "cbr") {
      PANIC_IF(!_mod.get_bb(ins.args[2].substr(1)) ||
                   !_mod.get_bb(ins.args[3].substr(1)),
               "cbr: invalid label name");
    }
  }
}
...
std::unique_ptr<IModule> mod2imod(const Module &mod) {
  ...
  // entry_bb.ins().push_back(Ins::parse("b @" + mod.code[0].label_defs[0]));
