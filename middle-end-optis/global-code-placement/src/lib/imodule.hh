...
private:
  ...
  mutable std::size_t _order; // relative position in module

  friend class IModule;

  BasicBlock(const IModule &mod, bb_id_t id, const std::string &label,
             const std::vector<Ins> &ins, std::size_t order)
      : _mod(mod), _ins(ins), _id(id), _label(label), _order(order) {}
};
...
public:
  ...
  // - the module must have an entry bb, which in the first in order
  void check() const;
  ...
  void bb_order_change(const std::vector<const BasicBlock *> &new_order);

  // Swawp order of 2 basic blocks
  void bb_order_swap(const BasicBlock *b1, const BasicBlock *b2);
