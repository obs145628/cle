...
std::unique_ptr<Dir> Dir::parse(const std::string &str) {
  ...
  auto nend = str.find(' ');
  if (nend == std::string::npos)
    return std::make_unique<Dir>(std::vector<std::string>{str.substr(1)});
  ...
  for (auto &arg : args)
    arg = utils::str::trim(arg);
  return std::make_unique<Dir>(args);
}
...
Module Module::clone() const {
  ...
  for (const auto &decl : decls) {
    auto ins = dynamic_cast<const Ins *>(decl.get());
    auto dir = dynamic_cast<const Dir *>(decl.get());

    std::unique_ptr<Decl> cpy;
    if (ins)
      cpy = std::make_unique<Ins>(ins->args);
    else if (dir)
      cpy = std::make_unique<Dir>(dir->args);
    else
      assert(0);

    cpy->comm_eol = decl->comm_eol;
    cpy->comm_pre = decl->comm_pre;
    cpy->label_defs = decl->label_defs;
    res.decls.push_back(std::move(cpy));
  }
