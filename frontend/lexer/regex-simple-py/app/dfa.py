...
class DFA:
    ...
    def __init__(self, states_count, start_state, final_states, err_state):
    ...
    def save_dot(self, out_path):
        ...
        with open(out_path, 'w') as f:
            ...
            for s0 in range(self.sc):
                if s0 == self.err:
                    continue
                for c, s1 in self.trans[s0].items():
                    if s1 == self.err: #ignore error state
                        continue
                    f.write('{} -> {} [ label="{}" ];\n'.format(s0, s1, c))
    ...
    def check(self, alpha):
        ...
        for tr in self.trans:
            if len(tr) > 0: #ignore removed states
                assert sorted(alpha.larr) == sorted(tr.keys())
