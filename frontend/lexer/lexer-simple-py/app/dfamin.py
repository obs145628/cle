...
class DFAMinimizer:
    ...
    def find_pairs(self):
        ...
        # or if both are finals but for different tags
        for s0 in range(self.dfa.sc):
            for s1 in range(self.dfa.sc):
                s0_tag = self.dfa.finals[s0] if s0 in self.dfa.finals else None
                s1_tag = self.dfa.finals[s1] if s1 in self.dfa.finals else None
                if s0_tag != s1_tag:
                    g.add_edge(s0, s1)
